...
def get_init_fn(flags):
    ...
    if flags.checkpoint_path is None:
        return None
    # Warn the user if a checkpoints exists in the train_dir. Then ignore.
    if tf.train.latest_checkpoint(flags.train_dir):
        tf.logging.info(
            'Ignoring --checkpoint_path because a checkpoints already exists in %s'
            % flags.train_dir)
        return None
